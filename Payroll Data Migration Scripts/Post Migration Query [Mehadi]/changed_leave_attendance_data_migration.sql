...
SELECT VM.application_date AS doc_date,
    U.pin AS employee_code,
    IEI.operating_location_code AS oper_location_code,
    20 AS work_away_type,
    VM.movement_place AS work_address,
    --VM.purpose::varchar AS deliverables,
    NULL AS deliverables,
    10 AS destination_type,
    ROW_NUMBER() OVER(PARTITION BY VM.employee_id,DATE(VM.from_date_time),DATE(VM.to_date_time) ORDER BY VM.ID DESC)row_num
...
SELECT
    ...
    , NOW()::timestamp with time zone AS last_modified_date,
    NULL AS doc_no,
    VM.doc_date::timestamp AS doc_date,
    VM.employee_code::varchar AS employee_code,
    VM.oper_location_code::varchar AS oper_location_code,
    20::INTEGER AS work_away_type,
    VM.work_address::varchar AS work_address,
...
--                                        where EXTRACT('YEAR' FROM to_date_time) >= 2024
--                                        order by a desc;
...
SELECT
    ...
    IOI.id::UUID AS work_away_office_imp_id,
    VM.movement_place::varchar AS visit_location
FROM payroll_leave_attend_data.visit_master VM
INNER JOIN payroll_emp_data.users U ON VM.employee_id = U.id
INNER JOIN BASE_CTE B ON U.pin = B.pin
...
INNER JOIN public.integ_work_away_office_imp IOI ON U.pin = IOI.employee_code AND DATE(IOI.doc_date) = DATE(VM.application_date)
INNER JOIN public.integ_employee_imp IEI ON U.pin = IEI.employee_code
WHERE VM.leavetype_id = 24
  AND EXTRACT('YEAR' FROM VM.to_date_time) >= 2024
  AND VM.duration > 0
  AND VM.is_international_travel = false
  AND DL.application_type = 'Visit' AND VM.status = 1 AND VM.is_deleted <> 1 AND VM.is_manual = 0;
...
SELECT TA.date_created AS doc_date,
    ECI.pin_no AS employee_code,
    IEI.operating_location_code AS oper_location_code,
    20 AS work_away_type,
    TA.destination AS work_address,
    --TA.purpose_of_travel::varchar AS deliverables,
    NULL AS deliverables,
    20 AS destination_type,
    ROW_NUMBER() OVER(PARTITION BY TA.employee_core_info_id,DATE(TA.official_visit_from_date),DATE(TA.official_visit_to_date) ORDER BY TA.id DESC)row_num
...
SELECT
    ...
    , NOW()::timestamp with time zone AS last_modified_date,
    NULL AS doc_no,
    TA.doc_date::timestamp AS doc_date,
    TA.employee_code::varchar AS employee_code,
    TA.oper_location_code::varchar AS oper_location_code,
    20::INTEGER AS work_away_type,
    TA.work_address::varchar AS work_address,
    NULL::varchar AS deliverables,
...
WITH RECURSIVE date_series AS (
    ...
    AND EXTRACT('YEAR' FROM official_visit_to_date) >= 2024 AND status_id not in (2, 3, 5, 8)
    UNION ALL
    SELECT
        employee_core_info_id,
         (travel_date + INTERVAL '1 day')::DATE,
        official_visit_to_date::DATE,
        date_created
    FROM date_series
    WHERE travel_date < official_visit_to_date::DATE
),BASE_CTE AS (
    SELECT eci.id AS Employee_ID
    FROM payroll_emp_data.employee_information_update e
    INNER JOIN payroll_emp_data.employee_core_info eci ON e.employee_core_info_id=eci.id
...
SELECT
    ...
    , '1'::varchar AS last_modified_by
    , NOW()::timestamp with time zone AS last_modified_date,
    DS.travel_date::timestamp AS attend_date,
    IWAOI.id::UUID AS work_away_office_imp_id
FROM payroll_leave_attend_data.travel_application TA
INNER JOIN payroll_emp_data.employee_core_info ECI ON TA.employee_core_info_id = ECI.id
INNER JOIN BASE_CTE B ON ECI.id = B.Employee_ID
INNER JOIN date_series DS ON TA.employee_core_info_id = DS.employee_core_info_id AND (DATE(DS.travel_date) >= DATE(TA.official_visit_from_date) AND DATE(DS.travel_date) <= DATE(TA.official_visit_to_date)) AND DS.date_created = TA.date_created
INNER JOIN public.integ_work_away_office_imp IWAOI ON ECI.pin_no = IWAOI.employee_code AND DATE(IWAOI.doc_date) = DATE(TA.date_created)
WHERE TA.domain_status_id = 1
  AND EXTRACT('YEAR' FROM TA.official_visit_to_date) >= 2024 AND TA.status_id not in (2, 3, 5, 8);
...
SELECT VM.application_date AS doc_date,
    U.pin AS employee_code,
    IEI.operating_location_code AS oper_location_code,
    10 AS work_away_type,
    VM.movement_place AS work_address,
    --VM.purpose::varchar AS deliverables,
    ROW_NUMBER() OVER(PARTITION BY VM.employee_id,DATE(from_date_time),DATE(to_date_time) ORDER BY VM.id DESC)row_num
FROM payroll_leave_attend_data.visit_master VM
INNER JOIN payroll_emp_data.users U ON VM.employee_id = U.id
INNER JOIN BASE_CTE B ON U.pin = B.pin
--INNER JOIN public.mdg_empl_profile IEI ON U.pin = IEI.employee_code
...
SELECT
    ...
    VM.employee_code::varchar AS employee_code,
    VM.oper_location_code::varchar AS oper_location_code,
    VM.work_away_type::INTEGER AS work_away_type,
    VM.work_address::varchar AS work_address,
    --VM.purpose::varchar AS deliverables,
    NULL AS deliverables,
...
WHERE VM.leavetype_id <> 24
  AND EXTRACT('YEAR' FROM VM.to_date_time) >= 2024
  AND VM.duration > 0
  AND DL.flag = 'WFA' AND VM.status = 1 AND VM.is_deleted <> 1 AND VM.is_manual = 0;
