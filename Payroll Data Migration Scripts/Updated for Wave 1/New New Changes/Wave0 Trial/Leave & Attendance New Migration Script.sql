...
INNER JOIN payroll_leave_attend_data.employee_records_wave0 ER ON U.pin = ER.pin
WHERE EXTRACT('YEAR' FROM LM.to_date) >= 2025
    AND LM.leave_duration > 0 AND LM.status = 1 AND LM.is_deleted <> 1 AND LM.is_manual = 0
    and lm.leave_type_id <> 6 AND LM.approval_action_time < '2025-08-27 21:00:00.000000'
),Leave_Others_Dup AS(
  SELECT
    LO.id,
...
WHERE EXTRACT('YEAR' FROM LO.to_date) >= 2025
    ...
    and lo.leave_type_id <> 6  AND LO.approval_action_time < '2025-08-27 21:00:00.000000'
)
SELECT
...
WHERE EXTRACT('YEAR' FROM LM.to_date) >= 2025
    ...
    AND LM.id NOT IN(SELECT id FROM Leave_Master_Dup WHERE from_rn >=2 OR to_rn >=2)
     AND LM.approval_action_time < '2025-08-27 21:00:00.000000'
...
AND LO.id NOT IN(SELECT id FROM Leave_Others_Dup WHERE from_rn >=2 OR to_rn >=2)
 AND LO.approval_action_time < '2025-08-27 21:00:00.000000';
...
INNER JOIN payroll_leave_attend_data.employee_records_wave0 ER ON ECI.pin_no = ER.pin
...
WHERE VM.leavetype_id = 24
  ...
  AND VM.is_international_travel = false AND VM.status = 1 AND VM.is_deleted <> 1 AND VM.is_manual = 0
     AND VM.approval_action_time < '2025-08-27 21:00:00.000000'
)
SELECT
    gen_random_uuid()::uuid AS id
    , 0::integer AS version
...
WITH BASE_CTE AS (
    ...
    FROM public.integ_work_away_office_imp
...
AND V.approval_action_time < '2025-08-27 21:00:00.000000'
...
FROM public.mdg_empl_profile ep
INNER JOIN payroll_leave_attend_data.employee_records_wave0 ER ON ep.employee_code = ER.pin
LEFT JOIN public.mdg_holiday_calendar hc on ep.holiday_calendar_id = hc.id
LEFT JOIN public.mdg_holiday_calendar_detail hcd on hc.id = hcd.holiday_calendar_id
),Generate_CTE AS(
SELECT
    wfa.id AS integ_work_away_office_imp,
    wfa.employee_code,
...
INNER JOIN visit_ranked_Initial v ON wfa.employee_code = v.pin_no
 ...
 AND wfa.rn = v.rn
INNER JOIN payroll_leave_attend_data.employee_records_wave0 ER ON V.pin_no = ER.pin
