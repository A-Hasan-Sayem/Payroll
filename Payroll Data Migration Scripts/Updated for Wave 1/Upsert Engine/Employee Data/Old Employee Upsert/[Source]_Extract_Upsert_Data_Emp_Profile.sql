INSERT INTO CDC.emp_upsert_staging
WITH RECURSIVE Employee_CTE AS(
SELECT eci.id AS Employee_ID, ECI.supervisor_id
FROM cdc.employee_information_update e
INNER JOIN cdc.employee_core_info eci ON e.employee_core_info_id=eci.id
...
FROM cdc.emp_core_info_history e
INNER JOIN cdc.employee_core_info eci ON e.employee_core_info_id::BIGINT=eci.id
WHERE e.value_type = 'Employee Status Change' AND date(e.create_date) > '2024-12-31'
UNION
SELECT eci.id AS Employee_ID, ECI.supervisor_id
FROM cdc.job_separation_proposal jsp
INNER JOIN cdc.employee_core_info eci ON jsp.employee_core_info_id=eci.id AND jsp.domain_status_id=1 AND eci.cur_job_status_id != 3 AND date(jsp.date_created) > '2024-12-31'
UNION
SELECT eci.id AS Employee_ID, ECI.supervisor_id
FROM cdc.employee_core_info eci
WHERE eci.cur_job_status_id = 3 AND eci.domain_status_id=1
    ...
    SELECT
        ...
        1 AS level
    FROM Employee_CTE I
    JOIN cdc.employee_core_info ECI ON I.supervisor_id = ECI.id
    ...
    JOIN cdc.employee_core_info ECI ON sh.supervisor_id = ECI.id
...
FROM cdc.employee_core_info WHERE pin_no = '01145674'
),NAME_CTE AS (
SELECT ECI.id,
...
FROM cdc.job_separation_proposal jsp
INNER JOIN cdc.employee_core_info eci
...
LEFT JOIN cdc.employee_core_info ECI2 ON ECI.supervisor_id = ECI2.id
LEFT JOIN public.employee_designation ED ON ECI.e_designation_id = ED.id
LEFT JOIN cdc.nationality N ON ECI.country_level_id = N.id
LEFT JOIN public.physical_office_info POI ON ECI.office_info_id = POI.id
LEFT JOIN public.project_info PI ON ECI.core_project_id = PI.id
LEFT JOIN BASIC_INFO_CTE BIC ON ECI.id = BIC.employee_info_id AND ROW_NUM = 1
LEFT JOIN cdc.employee_contact_info EC ON ECI.id = EC.employee_info_id AND EC.domain_status_id = 1
LEFT JOIN ADDRESS_CTE2 AC ON ECI.id = AC.employee_info_id
LEFT JOIN cdc.users U ON ECI.pin_no = U.pin
LEFT JOIN cdc.employeeleavegroupmapping EL ON U.id = EL.employee_id
LEFT JOIN cdc.employeewiseattendancepolicymapping EAP ON U.id = EAP.employee_id
LEFT JOIN cdc.emp_confirmation_date ECD ON ECI.pin_no = ECD.staffpin
LEFT JOIN cdc.gender G ON ECI.gender_id = G.id
LEFT JOIN cdc.employee_category EC2 ON ECI.emp_category_id = EC2.id
LEFT JOIN DUP_CTE DC ON ECI.pin_no = DC.Pin AND DC.row_num=1
WHERE ECI.last_updated > '2025-05-29' OR ED.last_updated > '2025-05-29'
OR POI.last_updated > '2025-05-29' OR PI.last_updated > '2025-05-29'
OR EC.last_updated > '2025-05-29' OR U.updated_at > '2025-05-29'
OR EL.updated_at > '2025-05-29' OR EAP.updated_at > '2025-05-29';

--SELECT last_updated FROM cdc.employee_core_info