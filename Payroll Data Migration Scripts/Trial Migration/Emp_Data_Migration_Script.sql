...
SELECT ECI.id,
    ...
    (string_to_array(first_name, ' '))[1] AS first_name,
      ...
      CASE
          ...
          END AS last_name
FROM payroll_emp_data.employee_core_info ECI
INNER JOIN Employee_CTE EC ON ECI.ID = EC.Employee_ID
),ADDRESS_CTE AS(
...
INNER JOIN Employee_CTE EC ON EA.employee_info_id = EC.Employee_ID
...
WHERE domain_status_id = 1
    ...
    , ECI.cur_job_status_id::integer AS employment_status    --need to send enum table in excel
    , ECI.email_address::VARCHAR AS email_company
...
FROM payroll_emp_data.employee_core_info ECI
INNER JOIN Employee_CTE ECC ON ECI.id = ECC.Employee_ID
...
--updated employee category to short_name
UPDATE public.integ_employee_imp I
SET empl_category_code = EC.short_name
FROM payroll_emp_data.employee_category EC
WHERE I.empl_category_code::BIGINT = EC.id;

--updated employment_status and employee_job_status as per discussion
UPDATE public.integ_employee_imp
SET
employee_job_status = employment_status,
employment_status = NULL
WHERE
employment_status IN (11, 7, 8, 9, 20, 13, 21, 14, 12, 17, 10, 15, 4, 18);


--updated emp wise last working date
WITH LAST_W_CTE AS (
...
eci.cur_job_status_id != 3 AND date(jsp.date_created) > '2023-12-31'
),DUP_CTE AS(
...
UPDATE public.integ_employee_imp AS I
SET last_working_day = D.last_working_date
FROM DUP_CTE D
WHERE I.employee_code = D.Pin AND D.row_num = 1;
...
--nationality updated
UPDATE public.integ_employee_imp
SET nationality_country = 'Bangladesh'

--employee job status updated while employment status nullified
UPDATE public.integ_employee_imp
SET
employee_job_status = employment_status,
employment_status = NULL
WHERE employee_job_status IS NULL




select * from public.integ_employee_imp; --88113