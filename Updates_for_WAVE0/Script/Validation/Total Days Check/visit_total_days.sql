----------------------------------- Visit Application Total days check START -----------------------------------------------
drop table if exists visit_app;
create temporary table visit_app AS
...
WHERE w.work_away_type = '20'; -- visit

drop table if exists visit_app_line;
create temporary table visit_app_line AS
...
from visit_app la
LEFT JOIN visit_app_line ll on la.app_id = ll.line_app_id
where la.total_days - ll.total_days <> 0 ;

----------------------------------- Visit Application Total days check END -----------------------------------------------