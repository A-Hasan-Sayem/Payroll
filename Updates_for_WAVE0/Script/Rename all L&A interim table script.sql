...
ALTER TABLE integ_leave_app_line_imp
ADD CONSTRAINT fk_integ_leave_app_line_imp_on_leave_app_imp
FOREIGN KEY (leave_app_imp_id)
REFERENCES integ_leave_app_imp_beta
ON DELETE CASCADE;
...
-- Add constraint to newly renamed beta master table
ALTER TABLE integ_work_away_office_line_imp
ADD CONSTRAINT fk_integ_work_away_office_line_imp_on_work_away_office_imp
FOREIGN KEY (work_away_office_imp_id)
REFERENCES integ_work_away_office_imp_beta
ON DELETE CASCADE;
...
(
    id  uuid not null
        constraint pk_integ_leave_app_imp_wave30-- added _beta, in order to avoid existing conflicts
            primary key,
...
(
    id                        uuid    not null
        constraint pk_integ_leave_app_line_imp_wave30 -- added beta to avoid conflicts
            primary key,
    version                   integer not null,
    created_by                varchar(255),
    created_date              timestamp,
    ...
    leave_app_imp_id          uuid    not null
        constraint fk_integ_leave_app_line_imp_on_leave_app_imp_wave30
...
create index idx_integ_leave_app_line_imp_leave_app_imp_wave30
...
(
    id                    uuid    not null
        constraint pk_integ_leave_op_balance_imp_wave30
            primary key,
    version               integer not null,
    created_by            varchar(255),
    created_date          timestamp,
...
(
    id                 uuid    not null
        constraint pk_integ_work_away_office_imp_wave30
            primary key,
    version            integer not null,
    created_by         varchar(255),
    created_date       timestamp,
...
(
    id                      uuid    not null
        constraint pk_integ_work_away_office_line_imp_wave30
            primary key,
    version                 integer not null,
    created_by              varchar(255),
    created_date            timestamp,
    ...
    work_away_office_imp_id uuid    not null
        constraint fk_integ_work_away_offc_line_imp_on_work_away_offc_imp_wave30
...
create index idx_integ_work_away_office_line_imp_work_away_office_imp_wave30
...
(
    id                  uuid not null
        constraint pk_integ_attend_missed_app_imp_wave30
            primary key,
